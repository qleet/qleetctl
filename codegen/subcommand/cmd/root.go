...
* update
* delete
For example, if you want to extend qleetctl to create a new QleetOS object
...
func init() {
	...
	rootCmd.Flags().BoolVarP(&optionalConfig, "optional-config", "o", false, "Make config optional for the qleetctl user")
}
...
var {{ .LowerCamelParent }}{{ .CamelName }}Cmd = &cobra.Command{
	Use: "{{ .Name }}",
{{- if .LoadConfig }}
	Example: "qleetctl {{ .Parent }} {{ .Name }} -c /path/to/config.yaml",
{{- else }}
	Example: "qleetctl {{ .Parent }} {{ .Name }}",
{{- end }}
